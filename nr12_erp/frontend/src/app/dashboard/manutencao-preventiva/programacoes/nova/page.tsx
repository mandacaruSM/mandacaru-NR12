...
export default function NovaProgramacaoManutencao() {
  ...
  const handleEquipamentoChange = (equipamentoId: number) => {
    ...
    if (equipamento) {
      const tipoEquipamentoId = equipamento.tipo
  ...
  const modelosFiltrados = selectedEquipamento
    ? modelos.filter((m) => {
        const tipoEquipamentoId = selectedEquipamento.tipo
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-lg shadow p-6 space-y-6">
        ...
        <div>
          ...
          <select
            ...
            className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent disabled:bg-gray-100 disabled:cursor-not-allowed text-gray-900 placeholder:text-gray-500"
            style={{ color: '#111827 !important' }}
          >
              ...
              <div className="text-sm space-y-1 text-gray-900">
                ...
                <div>
                  <span className="font-medium text-gray-900">
                    {selectedEquipamento.tipo_medicao === 'HORA' ? 'Horímetro' : 'Odômetro'} atual:
                  </span>{' '}
                  {selectedEquipamento.leitura_atual}{' '}
                  {selectedEquipamento.tipo_medicao === 'HORA' ? 'horas' : 'km'}
                </div>
              </div>
            </div>
          )}
