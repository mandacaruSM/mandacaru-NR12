...
export default function OrdemServicoDetalhesPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  async function confirmarConclusao() {
    ...
    // Validar horímetro se equipamento tiver
    if (os.equipamento && !horimetroFinal) {
      alert('Informe o horímetro/odômetro final do equipamento');
      return;
    }

    try {
      // Enviar horímetro final junto com conclusão
      await ordensServicoApi.update(Number(id), {
  ...
  function handleImprimir() {
    if (!os) return;
    gerarImpressaoProfissional('ordem_servico', os);
  }
      ...
      {showConclusaoModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 max-w-md w-full mx-4">
            <h2 className="text-xl font-bold text-gray-900 mb-4">Concluir Ordem de Serviço</h2>
            ...
            {os?.equipamento && (
              <div className="mb-4">
                ...
                <input
                  ...
                  onChange={(e) => setHorimetroFinal(e.target.value ? Number(e.target.value) : undefined)}
                  className="w-full px-3 py-2 border border-gray-300 rounded-md text-gray-900"
                  step="0.01"
                  placeholder="Leitura final do equipamento"
                  required
                />
                <p className="mt-1 text-sm text-gray-500">
                  Informe a leitura atual do horímetro ou odômetro do equipamento
                </p>
              </div>
            )}
              ...
              <div className="flex">
                ...
                <div className="ml-3">
                  <p className="text-sm text-yellow-700">
                    Ao concluir, uma conta a receber será criada automaticamente e todos os itens serão marcados como executados.
                  </p>
                </div>
              </div>
            </div>
