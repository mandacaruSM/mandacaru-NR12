...
export default function ContasPagarPage() {
  ...
  async function loadContas() {
    try {
      setLoading(true);
      const data = await financeiroApi.contasPagar.list(filters);
  ...
  async function loadResumo() {
    try {
      const data = await financeiroApi.contasPagar.resumo();
      setResumo(data);
    } catch (error) {
  ...
  return (
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        ...
        <Link
          href="/dashboard/financeiro/contas-pagar/nova"
      ...
      <div className="grid grid-cols-5 gap-4 mb-6">
        ...
        <div className="bg-white p-4 rounded-lg shadow">
          ...
          <p className="text-xl font-bold text-gray-900">R$ {Number(resumo.total_aberto || 0).toFixed(2)}</p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-lg shadow overflow-hidden">
        {loading ? (
          ...
          <div className="p-8 text-center text-gray-900">Nenhuma conta a pagar encontrada</div>
        ) : (
